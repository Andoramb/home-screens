...
const FIELD_DECISIONS: Record<string, FieldDecision> = {
  ...
  'fullscreen-photo.onedriveCount': 'fetch-only',
  'fullscreen-photo.onedriveFolderName': 'not-observable',
  ...
  'photo-slideshow.onedriveCount': 'fetch-only',
  'photo-slideshow.onedriveFolderName': 'not-observable',
...
const EXTRA_DISCRIMINATORS: Array<{ type: string; key: string; union?: string; members?: string[] }> = [
  ...
  { type: 'todo', key: 'completedPlacement', union: 'TodoCompletedPlacement' },
...
const DISCRIMINATOR_UNION_DECISIONS: Record<string, string> = {
  ...
  CalendarLegendMode: 'what one key strip lists; the legend-mode-groups (fullscreen) and legend-mode-people (small) rows cover both non-default modes, calendars is the default render',
