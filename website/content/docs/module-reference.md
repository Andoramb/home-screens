...
A day rule's background can also be a **picture**: pick from the art that ships with Home Screens (birthdays, Halloween, Christmas, and other quiet patterns) or add your own image or SVG under **Your pictures**. The picture is painted over the day cell exactly as it otherwise renders — behind the day's events, everywhere the calendar draws days — and **Art dimming** fades the picture itself: transparent parts of a PNG leave the cell untouched, so the cell keeps its normal look with the art laid on top.
