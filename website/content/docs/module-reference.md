...
- **schedule**: Per-day assignment via a `schedule` map of `memberId → number[]` (days-of-week, 0 = Sunday through 6 = Saturday). Lets you say "Alice on Mon/Wed, Bob on Tue/Thu, everyone on Fri–Sun" without creating separate chores. The editor and the remote both render a weekly grid UI for editing the schedule, and any day not covered by the schedule simply has no one assigned. A [family group](/docs/family#groups) can have a row of its own: `groupSchedule` is the same kind of map keyed by group ID, and everyone in the group has the chore on those days, including somebody who joins the group later. Every group with a row is also listed in `assigneeGroupIds`. A person who has their own row and is also in a scheduled group has the chore on either set of days. A chore in schedule mode also shows a small **(schedule)** label in the board when resolved to a single assignee, so you can tell it apart from a fixed one-person chore at a glance.
