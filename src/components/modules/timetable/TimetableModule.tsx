...
/**
 * The School Timetable module: one week card per person, on one wall.
 *
 ...
 * display's own zone), works out which week and which day each card should be
 * on, and then gets out of the way: `WeekCard` draws the week and
 * `timetable-layout.ts` decides everything about it.
 *
...
import type { Timetable, TimetableSchool } from '@/types/timetables';
...
import { formatDateSync, useFormattingLocale, useLocale, useTranslate } from '@/i18n';
...
const NO_CLOSURES: SchoolClosures = { schoolHolidays: [], publicHolidays: [] };
 ...
 */
function regionOf(school: TimetableSchool): string {
...
export default function TimetableModule({ config, style, timezone, timeFormat }: TimetableModuleProps) {
  ...
  const cards = useMemo<PersonCard[]>(
    () =>
      shown.map((entry) => {
        const closed = closures.get(regionOf(entry.school)) ?? NO_CLOSURES;
        return {
          ...entry,
    ...
    [shown, closures, subjects, now, timezone, config.nextWeekFromFriday],
  );
